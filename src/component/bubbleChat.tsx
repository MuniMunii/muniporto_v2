...
function BubbleChoices({handleSession,setShowChoices,setContactAppear,setIntroAppear,setShowCaseProject,introAppear,contactAppear,showCaseProject,index}:BubbleProps){
    return (
        <motion.div key={`choices-${introAppear}-${contactAppear}-${showCaseProject}`} initial={{opacity:0,scale:0.8}} animate={{opacity:1,scale:1,transition:{delay:index?index*0.2:1.4,duration:0.2}}} variants={chatAppearEffect} className="ml-auto w-fit h-fit flex justify-end z-10 isolate ">
...
function BubbleRight({text,id}:{text:string,id:string}){
    return (
        <motion.div initial={{opacity:0,scale:0.8}} key={`bubble-${text}`} id={id} animate={{opacity:1,scale:1,transition:{delay:0.2,duration:0.2}}} variants={chatAppearEffect} className="ml-auto w-fit h-fit flex justify-end z-10">
            ...
            <div className="right-tail"/>
            <p className="text-white rounded p-2 pointer-events-auto text-left">{text}</p>
